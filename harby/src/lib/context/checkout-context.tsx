...
// // Extend the existing ProductPreviewType
// interface ExtendedAddress extends Address, CustomAddressProps {}
...
interface CheckoutContext {
  ...
  setSavedAddress: (address: ExtendedAddress) => void
...
export const CheckoutProvider = ({ children }: CheckoutProviderProps) => {
  ...
  const setPaymentSession = (providerId: string) => {
    if (cart) {
      setPaymentSessionMutation(
        ...
        {
          onSuccess: ({ cart }:any) => {
            console.log("cart session payment", cart)
            setCart(cart)
            if (cart?.payment_session?.data?.url) {
              router.push(cart?.payment_session?.data?.url.toString())
            }
            // else{}
          },
        }
  ...
  const setSavedAddress = (address: ExtendedAddress) => {
  ...
  const setAddresses = (data: CheckoutFormValues) => {
    ...
    const payload: StorePostCartsCartReq = {
      shipping_address: {
        ...
        address_2: `${shipping_address.district}`,
    ...
    /* Billing Address is not required in HARBY */
    ...
    if (sameAsBilling.state) {
      ...
      payload.billing_address = payload.billing_address
    }
...
const mapFormValues = (
  ...
  return {
    shipping_address: {
      ...
      district:
        (cart?.shipping_address as ExtendedAddress)?.district ||
        (customerShippingAddress as ExtendedAddress)?.district ||
        "",
      country_code:
        currentCountry ||
