...
const PersonalizerTable = () => {
  ...
  const filteredProductsPersonalizer = products?.filter(
    (p) => p.metadata && p.metadata.isPersonalizer
  )
  ...
  const updateUrlFromFilter = (obj = {}) => {
    ...
    window.history.replaceState(`/a/personalizer`, "", `${`?${stringified}`}`)
  }
  ...
  const {
  ...
  } = useTable(
    {
      columns,
      // data: products || [],
      data: filteredProductsPersonalizer || [],
    ...
    >
      ...
      >
        {showList ? (
          ...
          <ProductOverview
          //@ts-ignore
            products={filteredProductsPersonalizer}
...
const ProductRow = ({ row, ...rest }) => {
  ...
  return (
    <Table.Row
      color={"inherit"}
      linkTo={`/a/personalizer/${product.id}`}
      actions={getActions()}
      {...rest}
    >
...
export default PersonalizerTable
