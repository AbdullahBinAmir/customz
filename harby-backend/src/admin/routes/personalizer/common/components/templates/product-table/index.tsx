...
const ProductTable = () => {
  ...
  const filteredProductsPersonalizer = products?.filter((p) => !p.metadata?.isPersonalizer)
  ...
  const updateUrlFromFilter = (obj = {}) => {
    ...
    window.history.replaceState(`/a/product`, "", `${`?${stringified}`}`)
  }
    ...
    >
      ...
      >
        {showList ? (
          ...
          //@ts-ignore
          <ProductOverview products={filteredProductsPersonalizer} toggleListView={setListView} />
        )}
      </Table>
    </TableContainer>
  )
