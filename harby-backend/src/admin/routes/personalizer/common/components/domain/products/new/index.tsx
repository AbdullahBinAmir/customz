    ...
    handleSubmit(async (data) => {
      ...
      if (data.media?.images?.length) {
        ...
        try {
          ...
          let errorMessage =
            "Images Size is too large to handle"
      ...
      mutate(payload, {
        onSuccess: async ({ product }) => {
          ...
          // );
          closeAndReset()
          navigate(`/a/product/${product.id}`)
        },
        onError: (err) => {
          notification("Error", getErrorMessage(err), "error")
        },
      })
      // console.log(data.variants.entries[0].options)
      // console.log(data.variants.entries[0].stock.inventory_quantity)
    })
