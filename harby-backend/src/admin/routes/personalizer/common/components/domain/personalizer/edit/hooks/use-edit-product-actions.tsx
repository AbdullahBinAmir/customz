...
const useEditProductActions = (productId: string) => {
  ...
  const onDelete = async () => {
    const shouldDelete = await dialog({
      heading: "Delete Personalizer",
      text: "Are you sure you want to delete this personalizer product?",
    })
    if (shouldDelete) {
      deleteProduct.mutate(undefined, {
        onSuccess: () => {
          notification("Success", "Product deleted successfully", "success")
          navigate("/a/personalizer/")
        },
        onError: (err) => {
  ...
  const onAddVariant = (
    ...
    // let values = payload.title.split("/")
    addVariant.mutate({...payload,options:[{
      option_id: option_id,
      value: payload.title
    }]}, {
      onSuccess: () => {
