...
const NewProduct = ({ onClose }: Props) => {
  ...
  // async function updateInventory(selectedOptions: Array<{ title: string, option: { value: string } }>, qty: number) {
  //   try {
    ...
    handleSubmit(async (data) => {
      ...
      mutate(payload, {
        onSuccess: async ({ product }) => {
          await createStockLocationsForVariants(product.variants,optionsToStockLocationsMap)
          // await Promise.all(
          //   data.variants.entries.map(entry => {
          //     return updateInventory(entry.options,entry.stock.inventory_quantity);
          //   })
          // );
          closeAndReset()
          navigate(`/a/personalizer/${product.id}`)
        },
        onError: (err) => {
        ...
        <FocusModal.Main className="no-scrollbar flex w-full justify-center py-16">
          <div className="small:w-4/5 medium:w-7/12 large:w-6/12 max-w-[700px]">
            <Accordion defaultValue={["general"]} type="multiple">
              ...
              <Accordion.Item title="Icons" value="media">
                <p className="inter-base-regular mb-large text-grey-50">
...
const createPayload = (
  ...
  const payload: AdminPostProductsReq = {
    // customStatus: "",
    // metadata: { isPersonalizer: data.general.isPersonalizer },
    metadata: { isPersonalizer: true, isEmroidery:data.general.isEmbroidery, embroideryText:data.general.embroideryText, isPrinting: data.general.isPrinting },
