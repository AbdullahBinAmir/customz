...
import Metadata, {
  MetadataField,
} from "../../../../personalizer/common/components/organisms/metadata"
...
import { createFulfillment } from "../../../../personalizer/common/services/customApi"
...
const CreateFulfillmentModal: React.FC<CreateFulfillmentModalProps> = ({
  ...
  const createFulfillmentCustom = async () => {
    ...
    if(res.status !==200){
      console.log(res.data.axiosError.message)
      notification("Error", res.data.axiosError.message, "error")
    }
    if (res?.status === 200) {
